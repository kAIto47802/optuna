...
def _get_upper_bound_set(
    ...
    def update(
        ...
        is_dominated = [np.all(sol < ub) for ub in ubs]
        if not any(is_dominated):
            return ubs, dps
            ## return

        ## dominated_dps = dps[is_dominated]  # The set `A` in Line 5 of Alg. 2.
        # dominated_dps = np.stack([dp for dp, is_dom in zip(dps, is_dominated) if is_dom], axis=0)
        ## print("2:", dominated_dps.shape)
        ### n_bounds = sum(is_dominated)
        dominated_dps = np.stack(
            list(compress(dps, is_dominated)), axis=0
        )  # The set `A` in Line 5 of Alg. 2.
        ...
        dominated_ubs = np.stack(list(compress(ubs, is_dominated)), axis=0)
        # dominated_ubs = np.stack([ub for ub, is_dom in zip(ubs, is_dominated) if is_dom], axis=0)
        new_ubs = dominated_ubs[ubs_indices_to_update]
        ...
        # ubs.append(new_ubs)
        # dps.append(new_dps)
        non_dominated_ubs = [ub for ub, is_dom in zip(ubs, is_dominated) if not is_dom]
        non_dominated_dps = [dp for dp, is_dom in zip(dps, is_dominated) if not is_dom]
        non_dominated_ubs.extend([ub for ub in new_ubs])
        non_dominated_dps.extend([db for db in new_dps])
        return non_dominated_ubs, non_dominated_dps

    upper_bound_set = [ref_point]  # Line 1 of Alg. 2.
    def_point = np.full((n_objectives, n_objectives), -np.inf)  # z^k(z^r) = \hat{z}^k
    def_point[objective_indices, objective_indices] = ref_point  # \hat{z}^k is a dummy point.
    def_points = [def_point]
    ...
    return np.stack(upper_bound_set, axis=0), np.stack(def_points, axis=0)
...
def get_non_dominated_box_bounds2(
