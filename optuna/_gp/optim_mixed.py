...
def _exhaustive_search(
    ...
    fvals = eval_acqf_with_constraints_no_grad(acqf_params, constraints_acqf_params, all_params)
...
def optimize_acqf_mixed(
    ...
    *,
    ...
    f_vals = eval_acqf_with_constraints_no_grad(acqf_params, constraints_acqf_params, sampled_xs)
    ...
    for x_warmstart in np.vstack([sampled_xs[chosen_idxs, :], warmstart_normalized_params_array]):
        x, f = local_search_mixed(acqf_params, constraints_acqf_params, x_warmstart, tol=tol)
        if f > best_f:
            best_x = x
            best_f = f
